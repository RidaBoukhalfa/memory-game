...
const categories = {
  animals: import.meta.glob('./Animals/*.{png,jpg,jpeg,webp}', { eager: true }),
  fruits: import.meta.glob('./Fruits/*.{png,jpg,jpeg,webp}', { eager: true }),
  flags: import.meta.glob('./Flags/*.{png,jpg,jpeg,webp}', { eager: true }),
  school: import.meta.glob('./images/*.{png,jpg,jpeg,webp}', { eager: true }),
  letters: import.meta.glob('./EnglishLetters/*.{png,jpg,jpeg,webp}', { eager: true }),
};

function getFormattedImages(files) {
  return Object.entries(files).map(([path, module]) => {
    const fileName = path.split('/').pop().replace(/\.(png|jpe?g|webp)$/, '');
...
function CustomGame() {
  ...
  const initializeGame = useCallback(() => {
    ...
    for (const cat in selection) {
      const count = selection[cat];
      if (count > 0) {
        const imgs = getFormattedImages(categories[cat]);
        const selected = getRandomSamples(imgs, count);
        allImages.push(...selected);
      }
    }

    const duplicated = allImages.flatMap((img, index) => [
      { ...img, id: index * 2 },
      { ...img, id: index * 2 + 1 },
    ]);
  ...
  useEffect(() => {
    if (!selection) {
      navigate('/CustomRandom'); // fallback if accessed directly
    } else {
      initializeGame();
    }
  }, [selection, initializeGame, navigate]);
